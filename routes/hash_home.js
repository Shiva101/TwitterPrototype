...
exports.checksignin = function(req,res)
{
	...
	{
		//var getUser="select * from users where username='"+ username + "' and password='" + hashedpassword +"'";
		...
		else 
		{
			if(results.length > 0){
				...
				// password correct?
				 if (bcrypt.compare (password, 	jsonParse.password) == true);{
...
exports.afterSignIn = function (req,res)
{
	...
	var getUser="select * from users where username='"+req.param("inputUsername")+"' and password='" + hashedpassword +"'";
	console.log("Query is:"+getUser);
